...
func Manejadores() {
	...
	router.HandleFunc("/login", middlew.ChekDB(routers.Login)).Methods("POST")
	router.HandleFunc("/verPerfil", middlew.ChekDB(middlew.ValidaJwt(routers.VerPerfil))).Methods("POST")
